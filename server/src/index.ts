...
const sequelize = new Sequelize('bmi_db',
 'root',
 '',
  {
    host: 'localhost',
...
app.post("/register", async (c) => {
  ...
  if(!request_body) {
    return c.json({
      message: "Please provide the request body in order to register an account.",
      data: null
    })
  }

  const does_user_exist = await User.findOne({where: {
...
app.post("/login", async(c) => {
  ...
  return c.json({message: "You logged in successfully.", data: does_user_exist})
})
